...
The findings above were built into JackStrawNYC/gmseat on the branch `claude/baseball-gm-ui-audit-wskrme` (apps/web only; no engine change). The same driver re-shot every screen against that build: the screenshots are in `screens-after/` with the same names, and the measurements in `after/metrics.json`. The league is a different seed, so the names and the dates differ; the state is the same (mid-season, a trade offer on the desk, a walk-year starter on the player page). The repo's smoke script, the two-season tour and the unit tests pass on the build, and the tour's baseline manifest was rewritten from that green run.

What changed, against the findings. The desk panel is the first thing under the title; the five stat cards are gone and run diff and home/away ride on the title line; the division table is club, W-L, GB; the owner card is one line with the letter and the mandate behind a disclosure; the departments and dollar cards live in the club's Finances section; messages dated after the last reading carry a new badge. The header is one status row with Advance, Week and To next stop, a More menu holding Sim season, Export and Roster file, a single busy label, a status word for saving instead of a row, and a ticker that opens to the club's last and next game with the league one click away; a phone gets a tab bar (Desk, Your club, Trades, Menu). The club page opens on the roster editor with finances last. The trades page puts the deal and the verdict above the asset tables, folds the board's log, shows six columns by default with three more behind a toggle, and says Accept in both places. Every DataTable can carry a default column set with a remembered More columns toggle, applied to league stats, the draft board, prospects, the farm, players, the market, a rival's lineup and the staff card. The player page lifts extension talks to the first panel in a walk year and no longer prints the contract's kind twice. The explainer paragraphs open once and then fold to a line. The roster editor's selects are narrower and the injury tag is the injury and the time.

Measured on the same screens, before and after, from the DOM at capture. Header: 155px to 125px at 1440 (17% to 14% of the viewport), 280px to 174px at 390 (33% to 21%). Desk messages above the fold at 390: none before, two after, and the first decision row sits at about y=545 (`screens-after/10-desk-390.png`). Club page height at 1440: 8,435px to 4,863px, with the roster editor's Save bar at y=355 (`screens-after/20-club-1440.png`). Trades page height: 2,098px to 1,571px, the deal panel at y=450 above the tables, asset tables 10 and 9 columns to 7 and 6 (`screens-after/40-trade-negotiation-1440-full.png`). Default columns: league stats 25 to 7, draft board 14 to 8, farm 10 to 7, players 10 to 8, market 8 to 7; ink coverage on the stats fold 44% to 23%, on the draft fold 31% to 24% (`screens-after/84-stats-1440.png`, `screens-after/51-draft-1440.png`). Player page: the extension panel is first and bordered live (`screens-after/30-player-1440.png`). The phone week advance, unreachable in the first run, captured this time (`screens-after/64-week-after-390.png`).

Not done in this pass, and still open. Tables on a phone still scroll inside their panels rather than becoming card rows (the roster editor's stat columns and the trade tables' salary column sit off the right edge at 390). The player status line component exists and is used on the club's reserve and injured lists and the trade partner's shopping line; the desk's message prose, the card art, the depth chart and the draft lists keep their own formats. The Sim season confirmation is still the browser's own dialog. The bottom tab bar's Menu opens the same twenty-link drawer. The stats page's saved view is per browser, not per league.
