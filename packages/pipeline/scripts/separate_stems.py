...
TWO_STEM_OUTPUTS = ["vocals.wav", "no_vocals.wav"]
...
def separate(config: dict) -> dict:
    ...
    two_stems = config.get("twoStems", False)
    ...
    try:
        ...
        demucs_out = tmp_dir / model_name / track_name

        if not demucs_out.exists():
            return {"ok": False, "error": f"Demucs output dir not found: {demucs_out}"}

        expected_stems = TWO_STEM_OUTPUTS if two_stems else CANONICAL_STEMS
        ...
        if len(stems_found) != len(expected_stems):
            return {
                "ok": False,
                "error": f"Expected {len(expected_stems)} stems, got {len(stems_found)}: {stems_found}",
                "elapsed": elapsed,
            }
