...
/**
 * Build a show-level shader pool: ~10-12 modes that the entire show draws from.
 * A real VJ commits to a visual palette — this prevents 69 shaders from creating
 * incoherent visual language across a show.
 *
 * Algorithm:
 * 1. Collect each song's top modes (show modes + defaultMode)
 * 2. Union into candidates, ranked by cross-song reference count
 * 3. Prune to targetSize (most-shared survive)
 * 4. Deterministic: same seed + same songs = same pool
 */
export function buildShowShaderPool(
  songs: Array<{ title: string; defaultMode: VisualMode }>,
  showSeed: number,
  targetSize = 12,
): VisualMode[] {
  // Collect all candidate modes with cross-song reference counts
  const refCounts = new Map<VisualMode, number>();

  for (const song of songs) {
    const identity = lookupSongIdentity(song.title);
    const modes = new Set<VisualMode>();

    // Always include the song's default mode
    modes.add(song.defaultMode);

    // Add show modes from identity
    if (identity?.preferredModes?.length) {
      const showModes = getShowModesForSong(identity.preferredModes, showSeed, song.title);
      for (const m of showModes) modes.add(m);
    }

    for (const m of modes) {
      refCounts.set(m, (refCounts.get(m) ?? 0) + 1);
    }
  }

  // Sort by reference count (most-shared first), then deterministic tiebreak
  const rng = seeded(showSeed + 0x5001);
  const sorted = Array.from(refCounts.entries())
    .map(([mode, count]) => ({ mode, count, jitter: rng() }))
    .sort((a, b) => b.count - a.count || a.jitter - b.jitter);

  // Take top targetSize
  const pool = sorted.slice(0, targetSize).map((s) => s.mode);
