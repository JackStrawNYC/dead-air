...
void main() {
  ...
  if (wasHit) {
    ...
    float d0 = maMap(hitPos, energy, bass, maTime, tension,
                     twistMult, morphAmt, sJam, sSpace, climB,
                     halfW, halfH, majorR);
    vec3 norm = normalize(vec3(
      maMap(hitPos + eps.xyy, energy, bass, maTime, tension, twistMult, morphAmt, sJam, sSpace, climB, halfW, halfH, majorR) - d0,
      maMap(hitPos + eps.yxy, energy, bass, maTime, tension, twistMult, morphAmt, sJam, sSpace, climB, halfW, halfH, majorR) - d0,
      maMap(hitPos + eps.yyx, energy, bass, maTime, tension, twistMult, morphAmt, sJam, sSpace, climB, halfW, halfH, majorR) - d0
    ));
    ...
    for (int j = 1; j < 4; j++) {
      ...
      float aoSample = maMap(hitPos + norm * aoDist, energy, bass, maTime, tension,
                             twistMult, morphAmt, sJam, sSpace, climB,
                             halfW, halfH, majorR);
      maAO -= (aoDist - aoSample) * (0.35 / float(j));
    }
    maAO = clamp(maAO, 0.12, 1.0);
    ...
    // ─── Compose surface ───
    col = surfaceCol * (0.04 + diffuse * 0.30 + fillDiff) * maAO;
