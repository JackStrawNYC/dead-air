...
// ---- Normal ----
vec3 lmCalcNormal(vec3 pos, float time, float energy, float tension, float bass,
                   float stability, float rotation, float coherence) {
  float eps = 0.003;
  int dummyId;
  float ref = lmSceneSDF(pos, time, energy, tension, bass, stability, rotation, coherence, dummyId);
  return normalize(vec3(
    lmSceneSDF(pos + vec3(eps, 0, 0), time, energy, tension, bass, stability, rotation, coherence, dummyId) - ref,
    lmSceneSDF(pos + vec3(0, eps, 0), time, energy, tension, bass, stability, rotation, coherence, dummyId) - ref,
    lmSceneSDF(pos + vec3(0, 0, eps), time, energy, tension, bass, stability, rotation, coherence, dummyId) - ref
  ));
}

// ---- Occlusion ----
float lmCalcOcclusion(vec3 pos, vec3 nrm, float time, float energy, float tension,
                       float bass, float stability, float rotation, float coherence) {
  float occl = 0.0;
  float weight = 1.0;
  int dummyId;
  for (int i = 1; i <= 4; i++) {
    float sd = float(i) * 0.1;
    float sdf = lmSceneSDF(pos + nrm * sd, time, energy, tension, bass, stability, rotation, coherence, dummyId);
    occl += weight * max(sd - sdf, 0.0);
    weight *= 0.5;
  }
  return clamp(1.0 - occl * 4.0, 0.0, 1.0);
}
...
void main() {
  ...
  if (didCollide) {
    ...
    vec3 nrm = lmCalcNormal(collidePos, sceneTime, energy, tension, bass, stability, rotation, coherence);
    float occl = lmCalcOcclusion(collidePos, nrm, sceneTime, energy, tension, bass, stability, rotation, coherence);
