...
void main() {
  ...
  if (didHit) {
    ...
    vec2 eps2 = vec2(0.002, 0.0);
    float d0 = dcMap(hitPos, dcTime, energy, bass, drumOn, melodicP, sJam, sSpace, climB).x;
    vec3 norm = normalize(vec3(
      dcMap(hitPos + eps2.xyy, dcTime, energy, bass, drumOn, melodicP, sJam, sSpace, climB).x - d0,
      dcMap(hitPos + eps2.yxy, dcTime, energy, bass, drumOn, melodicP, sJam, sSpace, climB).x - d0,
      dcMap(hitPos + eps2.yyx, dcTime, energy, bass, drumOn, melodicP, sJam, sSpace, climB).x - d0
    ));
    ...
    for (int j = 1; j < 4; j++) {
      ...
      float aoSample = dcMap(hitPos + norm * aoDist, dcTime, energy, bass, drumOn, melodicP, sJam, sSpace, climB).x;
      ambOcc -= (aoDist - aoSample) * (0.4 / float(j));
    }
    ambOcc = clamp(ambOcc, 0.12, 1.0);
