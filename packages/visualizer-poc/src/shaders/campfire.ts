...
// ============================================================
vec3 cf2Normal(vec3 pos) {
  float eps = 0.002;
  float d = cf2Map(pos).x;
  return normalize(vec3(
    cf2Map(pos + vec3(eps, 0.0, 0.0)).x - d,
    cf2Map(pos + vec3(0.0, eps, 0.0)).x - d,
    cf2Map(pos + vec3(0.0, 0.0, eps)).x - d
  ));
}
...
// Ambient occlusion
// ============================================================
float cf2AO(vec3 pos, vec3 norm) {
  float occ = 0.0;
  float weight = 1.0;
  for (int i = 1; i <= 5; i++) {
    float dist = 0.01 + 0.04 * float(i);
    float sampleD = cf2Map(pos + norm * dist).x;
