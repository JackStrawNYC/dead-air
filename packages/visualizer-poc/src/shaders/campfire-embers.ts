...
vec3 ceNormal(vec3 pos, float timeVal) {
  float eps = 0.001;
  float d = ceMap(pos, timeVal);
  return normalize(vec3(
    ceMap(pos + vec3(eps, 0.0, 0.0), timeVal) - d,
    ceMap(pos + vec3(0.0, eps, 0.0), timeVal) - d,
    ceMap(pos + vec3(0.0, 0.0, eps), timeVal) - d
  ));
}
...
void main() {
  ...
  if (sceneHitFlag) {
    vec3 nrm = ceNormal(sceneHitPos, timeVal);
