...
void main() {
  ...
  if (didHitGeom) {
    ...
    float d0 = btMap(hitPos, bassShake, gapWidth, climaxOpen, flowTime);
    vec3 norm = normalize(vec3(
      btMap(hitPos + eps.xyy, bassShake, gapWidth, climaxOpen, flowTime) - d0,
      btMap(hitPos + eps.yxy, bassShake, gapWidth, climaxOpen, flowTime) - d0,
      btMap(hitPos + eps.yyx, bassShake, gapWidth, climaxOpen, flowTime) - d0
    ));
    ...
    for (int j = 1; j < 5; j++) {
      ...
      float aoSample = btMap(hitPos + norm * aoDist, bassShake, gapWidth, climaxOpen, flowTime);
      occl -= (aoDist - aoSample) * (0.35 / float(j));
    }
    occl = clamp(occl, 0.1, 1.0);
