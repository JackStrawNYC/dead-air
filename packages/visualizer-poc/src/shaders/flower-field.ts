...
// ============================================================
vec3 fflNormal(vec3 pos, float timeVal, float bass, float energy, float bloom,
               float sJam, float sSpace) {
  float eps = 0.002;
  float d = fflMap(pos, timeVal, bass, energy, bloom, sJam, sSpace).x;
  return normalize(vec3(
    fflMap(pos + vec3(eps, 0.0, 0.0), timeVal, bass, energy, bloom, sJam, sSpace).x - d,
    fflMap(pos + vec3(0.0, eps, 0.0), timeVal, bass, energy, bloom, sJam, sSpace).x - d,
    fflMap(pos + vec3(0.0, 0.0, eps), timeVal, bass, energy, bloom, sJam, sSpace).x - d
  ));
}
...
// Ambient occlusion
// ============================================================
float fflAO(vec3 pos, vec3 norm, float timeVal, float bass, float energy,
            float bloom, float sJam, float sSpace) {
  float occ = 0.0;
  float weight = 1.0;
  for (int i = 1; i <= 5; i++) {
    float dist = 0.01 + 0.04 * float(i);
    float sampleDist = fflMap(pos + norm * dist, timeVal, bass, energy, bloom, sJam, sSpace).x;
    occ += (dist - sampleDist) * weight;
    weight *= 0.7;
  }
...
void main() {
  ...
  if (marchResult.y < 0.0) {
    ...
    vec3 norm = fflNormal(hitPos, timeVal, bass, energy, bloomState, sJam, sSpace);
    ...
    // Ambient occlusion
    float ambOcc = fflAO(hitPos, norm, timeVal, bass, energy, bloomState, sJam, sSpace);
