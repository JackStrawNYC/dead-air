...
vec3 in2Normal(vec3 pos, float bass, float energy, float drumOnset, float tension,
               float flowTime, float floodLevel, float melodicPitch, float eruptionScale,
               float geyserTime) {
  vec2 offset = vec2(0.003, 0.0);
  float dist = in2Map(pos, bass, energy, drumOnset, tension, flowTime, floodLevel,
                      melodicPitch, eruptionScale, geyserTime).x;
  vec3 norm = vec3(
    in2Map(pos + offset.xyy, bass, energy, drumOnset, tension, flowTime, floodLevel,
           melodicPitch, eruptionScale, geyserTime).x - dist,
    in2Map(pos + offset.yxy, bass, energy, drumOnset, tension, flowTime, floodLevel,
           melodicPitch, eruptionScale, geyserTime).x - dist,
    in2Map(pos + offset.yyx, bass, energy, drumOnset, tension, flowTime, floodLevel,
           melodicPitch, eruptionScale, geyserTime).x - dist
  );
...
// ═══════════════════════════════════════════════════════════
// Ambient occlusion — cheap 5-sample hemisphere
// ═══════════════════════════════════════════════════════════

float in2Occlusion(vec3 pos, vec3 norm, float bass, float energy, float drumOnset,
                   float tension, float flowTime, float floodLevel, float melodicPitch,
                   float eruptionScale, float geyserTime) {
  float occ = 0.0;
  float weight = 1.0;
  for (int i = 0; i < 5; i++) {
    float dist = 0.02 + 0.06 * float(i);
    float sampled = in2Map(pos + norm * dist, bass, energy, drumOnset, tension,
                           flowTime, floodLevel, melodicPitch, eruptionScale, geyserTime).x;
...
void main() {
  ...
  if (marchFound) {
    vec3 norm = in2Normal(marchPos, bass, energy, drumOnset, tension,
                          flowTime, floodLevel, melodicPitch, eruptionScale, geyserTime);

    // Ambient occlusion
    float occVal = in2Occlusion(marchPos, norm, bass, energy, drumOnset, tension,
                                flowTime, floodLevel, melodicPitch, eruptionScale, geyserTime);
