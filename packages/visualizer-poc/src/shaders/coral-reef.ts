...
vec3 cr2Normal(vec3 pos, float time, float bassVal, float midsVal) {
  vec2 off = vec2(0.002, 0.0);
  float centerD = cr2Map(pos, time, bassVal, midsVal).x;
  return normalize(vec3(
    cr2Map(pos + off.xyy, time, bassVal, midsVal).x - centerD,
    cr2Map(pos + off.yxy, time, bassVal, midsVal).x - centerD,
    cr2Map(pos + off.yyx, time, bassVal, midsVal).x - centerD
  ));
}
...
float cr2Occlusion(vec3 pos, vec3 norm, float time, float bassVal, float midsVal) {
  float occl = 0.0;
  float scale = 1.0;
  for (int occIdx = 0; occIdx < 5; occIdx++) {
    float stepDist = 0.02 + float(occIdx) * 0.06;
    float sampled = cr2Map(pos + norm * stepDist, time, bassVal, midsVal).x;
...
void main() {
  ...
  // === SURFACE SHADING ===
  if (didHitSurface) {
    vec3 norm = cr2Normal(marchPos, flowTime, bassVal, midsVal);
    float occl = cr2Occlusion(marchPos, norm, flowTime, bassVal, midsVal);
