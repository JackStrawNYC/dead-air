...
vec3 leNormal(vec3 pos, float bassV, float energyV, float tempoV, float tensionV,
              float stabilityV, float climaxWarp, float sectionSpeedMul) {
  vec2 offset = vec2(0.002, 0.0);
  float d = leMap(pos, bassV, energyV, tempoV, tensionV, stabilityV, climaxWarp, sectionSpeedMul).x;
  return normalize(vec3(
    leMap(pos + offset.xyy, bassV, energyV, tempoV, tensionV, stabilityV, climaxWarp, sectionSpeedMul).x - d,
    leMap(pos + offset.yxy, bassV, energyV, tempoV, tensionV, stabilityV, climaxWarp, sectionSpeedMul).x - d,
    leMap(pos + offset.yyx, bassV, energyV, tempoV, tensionV, stabilityV, climaxWarp, sectionSpeedMul).x - d
  ));
}
...
void main() {
  ...
  if (totalDist < LE_MAX_DIST) {
    ...
    vec3 surfNorm = leNormal(surfPos, bassV, energyV, tempoV, tensionV, stabilityV, climaxWarp, sectionSpeedMul);
