...
vec3 st2CalcNormal(vec3 pos, float tunnelRadius, float twist, float flowTime, float bassBreath) {
  vec2 eps = vec2(0.003, 0.0);
  float d0 = st2SceneSDF(pos, tunnelRadius, twist, flowTime, bassBreath).x;
  return normalize(vec3(
    st2SceneSDF(pos + eps.xyy, tunnelRadius, twist, flowTime, bassBreath).x - d0,
    st2SceneSDF(pos + eps.yxy, tunnelRadius, twist, flowTime, bassBreath).x - d0,
    st2SceneSDF(pos + eps.yyx, tunnelRadius, twist, flowTime, bassBreath).x - d0
  ));
}

float st2CalcAO(vec3 pos, vec3 norm, float tunnelRadius, float twist, float flowTime, float bassBreath) {
  float occ = 0.0;
  float weight = 1.0;
  for (int i = 1; i <= 5; i++) {
    float dist = float(i) * 0.1;
    float sampled = st2SceneSDF(pos + norm * dist, tunnelRadius, twist, flowTime, bassBreath).x;
...
void main() {
  ...
  if (didHitSurface) {
    ...
    vec3 normal = st2CalcNormal(hitPos, tunnelRadius, twist, flowTime, bassBreath);
    float ambOcc = st2CalcAO(hitPos, normal, tunnelRadius, twist, flowTime, bassBreath);
