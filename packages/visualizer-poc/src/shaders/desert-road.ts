...
// ============================================================
vec3 drdNormal(vec3 pos, float timeVal, float bass) {
  float eps = 0.002;
  float d = drdMap(pos, timeVal, bass).x;
  return normalize(vec3(
    drdMap(pos + vec3(eps, 0.0, 0.0), timeVal, bass).x - d,
    drdMap(pos + vec3(0.0, eps, 0.0), timeVal, bass).x - d,
    drdMap(pos + vec3(0.0, 0.0, eps), timeVal, bass).x - d
  ));
}
...
// Ambient occlusion
// ============================================================
float drdAO(vec3 pos, vec3 norm, float timeVal, float bass) {
  float occ = 0.0;
  float weight = 1.0;
  for (int i = 1; i <= 5; i++) {
    float dist = 0.02 + 0.06 * float(i);
    float sampleDist = drdMap(pos + norm * dist, timeVal, bass).x;
...
void main() {
  ...
  if (marchResult.y < 0.0) {
    ...
    vec3 norm = drdNormal(hitPos, timeVal, bass);
    ...
    float ambOcc = drdAO(hitPos, norm, timeVal, bass);
