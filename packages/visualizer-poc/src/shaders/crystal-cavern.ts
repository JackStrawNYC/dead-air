...
// ============================================================
vec3 cvNormal(vec3 p, float bassVib, float climaxShatter) {
  vec2 eps = vec2(0.003, 0.0);
  float d = cvMap(p, bassVib, climaxShatter);
  return normalize(vec3(
    cvMap(p + eps.xyy, bassVib, climaxShatter) - d,
    cvMap(p + eps.yxy, bassVib, climaxShatter) - d,
    cvMap(p + eps.yyx, bassVib, climaxShatter) - d
  ));
}
...
// ============================================================
float cvAmbientOcclusion(vec3 p, vec3 n, float bassVib, float climaxShatter) {
  float occ = 0.0;
  float weight = 1.0;
  for (int i = 1; i <= 5; i++) {
    float fi = float(i);
    float dist = fi * 0.08;
    float d = cvMap(p + n * dist, bassVib, climaxShatter);
    occ += (dist - d) * weight;
    weight *= 0.6;
  }
...
void main() {
  ...
  if (marchHit) {
    vec3 pos = marchPos;
    vec3 norm = cvNormal(pos, bassVib, climaxShatter);
    float matID = cvMaterialID(pos, bassVib, climaxShatter);

    // === AMBIENT OCCLUSION ===
    float occl = cvAmbientOcclusion(pos, norm, bassVib, climaxShatter);
